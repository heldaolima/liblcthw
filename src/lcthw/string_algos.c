...
static inline void String_setup_skip_chars(size_t *skip_chars, const uchar* needle, ssize_t nlen)
{
    size_t i = 0;
    size_t last = nlen - 1;
...
static inline const uchar *String_base_search(
    const uchar * haystack,
    ssize_t hlen,
    const uchar *needle,
    ssize_t nlen,
    size_t *skip_chars
)
{
    ...
    check(nlen > 0, "nlen cannot be <= 0");
    check(hlen > 0, "hlen cannot be <= 0");
...
{
    const uchar* found = NULL;
    const uchar* haystack = (const uchar*) bdata(in);
    ssize_t hlen = blength(in);
    
    const uchar* needle = (const uchar*) bdata(what);
...
StringScanner *StringScanner_create(bstring in)
{
    ...
    scan->in = in;
    scan->haystack = (const uchar*) bdata(in);
...
{
    scan->needle = (const uchar*) bdata(tofind);
...
{
    scan->haystack = (const uchar*) bdata(scan->in);
...
int StringScanner_scan(StringScanner* scan, bstring tofind)
{
    ...
    if ((const uchar*)bdata(tofind) != scan->needle) {
    ...
    if (found) {
        found_at = found - (const uchar*) bdata(scan->in);
