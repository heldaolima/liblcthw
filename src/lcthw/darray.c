...
static inline int DArray_resize(DArray *array, size_t new_size)
{
    ...
    check(array->max > 0, "The new_size must be > 0");
...
int DArray_expand(DArray* array)
{
    ...
    check(DArray_resize(array, array->max + array->expand_rate) == 0, "Failed to expand arary to new size %d", array->max + (int) array->expand_rate);
