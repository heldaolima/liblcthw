...
int attempt_listen(struct addrinfo *info)
{
    ...
    int sock_fd = -1; //default fail
    int rc = -1;
    int yes = 1;

    sock_fd = socket(info->ai_family, info->ai_socktype, info->ai_protocol);
    check_debug(sock_fd != -1, "Failed to bind to address. Trying more.");

    rc = setsockopt(sock_fd, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(int));
    check_debug(sock_fd != -1, "Failed to set SO_REUSEADDR");

    rc = bind(sock_fd, info->ai_addr, info->ai_addrlen);
    check(rc == 0, "Failed to find socket.");
...
int server_listen(const char* host, const char* port)
{
    ...
    for (next_p = info; next_p != NULL; next_p = next_p->ai_next) {
        sock_fd = attempt_listen(next_p);
        if (sock_fd != -1)
            break;
    }

    check(sock_fd != -1, "All avaliable addresses failed.");
