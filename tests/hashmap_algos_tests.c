...
char* test_fnv1a()
{
    ...
    mu_assert(hash != 0, "Bad hash at test 1.");

    hash = Hashmap_fnv1a_hash(&test2);
    mu_assert(hash != 0, "Bad hash at test 2");

    hash = Hashmap_fnv1a_hash(&test3);
    mu_assert(hash != 0, "Bad hash at test 3");
...
char* test_distribution()
{
    ...
    mu_assert(table != NULL, "Failed to open table.txt");

    fprintf(table, "FNV\tA32\tDJB\n");
